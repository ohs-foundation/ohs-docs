...
slug: /ohs-player/setups/client-app/
sidebar_position: 2
...
This guide is step 3 of the [quick start](/ohs-player/setups/) for a usable integrated workflow: first prepare the shared Player environment and use the [Web Portal](/ohs-player/setups/web-portal/) to administer it, then evaluate the Client App in that prepared environment.
...
To add a field, update the `ViewDefinition`, build again, then use the regenerated field in the renderer. Read [Adapt UI configuration](/ohs-player/concepts/configuration/) before deciding whether the change belongs in declarative configuration, a renderer, or the reference application.
