...
slug: /ohs-player/setups/web-portal/
sidebar_position: 3
...
The Web Portal is a configurable, extensible web application for healthcare organizations to manage workforce hierarchies, user accounts, access controls, and configuration. It is step 2 of the [quick start](/ohs-player/setups/): use it after the shared Player environment is available and before running the Client App, so the environment has the users and administration data it needs.
...
[Run the Client App](/ohs-player/setups/client-app/) after the shared Player environment and Portal administration are ready.
