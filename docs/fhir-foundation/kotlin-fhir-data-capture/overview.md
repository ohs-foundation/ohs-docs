...
Kotlin FHIR Data Capture turns a FHIR Questionnaire into a working, validated form. Hand it a Questionnaire's JSON and it renders the questions with Material 3 components, applies the Questionnaire's behavior — skip logic, calculated values, answer options — validates the answers, and returns a `QuestionnaireResponse`. It implements FHIR's Structured Data Capture (SDC) patterns for R4, built on Compose Multiplatform so the same form code runs on Android, iOS, desktop, and the browser.
...
- **A single entry point**: the `Questionnaire` composable — Questionnaire JSON in, `QuestionnaireResponse` out through an `onSubmit` callback that runs validation first.
- **Built-in renderers** for the standard question types: text in single and multi-line, integer and decimal, boolean, date, time and date-time pickers, radio groups, checkboxes, dropdowns, autocomplete, dialog selection, sliders, quantities, phone numbers, attachments, groups, and display items.
- **SDC behavior driven by the Questionnaire itself**: `enableWhen` and enable-expressions, calculated and initial expressions, answer expressions and answer ValueSets, launch context, entry mode, paginated and review layouts — expressed in the Questionnaire resource, evaluated at runtime with [Kotlin FHIRPath](/fhir-foundation/kotlin-fhirpath/).
- **Validation** from the Questionnaire's constraints — required, min/max values, length, regex, and constraint extensions — surfaced in the UI before submission.
...
- **Not storage and not sync.** The library is purely in-memory; persisting the `QuestionnaireResponse` (or extracted resources) is your application's job — typically with [Kotlin FHIR Engine](/fhir-foundation/kotlin-fhir-engine/), on which this library deliberately does not depend.
- **R4 only**, matching the rest of the upper stack.
