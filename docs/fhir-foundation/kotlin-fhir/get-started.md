...
- For JVM projects: a Java 21 runtime — the current release candidate's JVM artifact targets Java 21.
...
Resources are plain Kotlin classes in `dev.ohs.fhir.model.r4`. FHIR primitives are wrapped types — a FHIR `string` is `FhirString`, a FHIR `date` is `Date` carrying a `FhirDate` value — so the structure of the spec is visible in the code:
...
The generated primitive types reuse FHIR's own names — the FHIR `string` type is literally `dev.ohs.fhir.model.r4.String` — so import them with an alias to avoid clashing with Kotlin's built-ins.
...
[Work with resources in more depth](/fhir-foundation/kotlin-fhir/working-with-resources/) — modification, dates and decimals, and choice types.
