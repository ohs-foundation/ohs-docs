...
description: The type-safe search DSL — filters, sorting, counts, and the boundaries of what is queryable offline.
...
The parameter name is the FHIR search parameter's name (`"name"`, `"birthdate"`, `"identifier"`, …). Comparable types take a prefix alongside the value, mirroring FHIR's search prefixes:
...
`search(xFhirQuery: String)` accepts an x-fhir-query string (`"Patient?name=Jo"`) for cases where the query arrives as data — configuration-driven UIs, for instance. Its translator supports plain parameter matching only: no modifiers, prefixes, chained parameters, global common parameters, or embedded FHIRPath. When you control the query at compile time, prefer the typed DSL.

## What is queryable

- Indexing happens at write time, driven by the standard R4 search-parameter definitions. Parameters whose FHIRPath expressions the [FHIRPath engine](/fhir-foundation/kotlin-fhirpath/) cannot evaluate — `resolve()`-based ones, for example — are skipped, so those parameters return no results rather than failing.
