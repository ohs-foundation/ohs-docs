...
- **SDC behavior driven by the Questionnaire itself.** `enableWhen` and enable-expressions, calculated and initial expressions, answer expressions and answer ValueSets, launch context, entry mode, and paginated and review layouts are expressed in the Questionnaire resource and evaluated at runtime with [Kotlin FHIRPath](/fhir-foundations/kotlin-fhirpath/).
