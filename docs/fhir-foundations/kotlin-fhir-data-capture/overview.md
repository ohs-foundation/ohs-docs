...
slug: /fhir-foundations/kotlin-fhir-data-capture/
sidebar_position: 10
...
- **SDC behavior driven by the Questionnaire itself.** `enableWhen` and enable-expressions, calculated and initial expressions, answer expressions and answer ValueSets, launch context, entry mode, and paginated layout are expressed in the Questionnaire resource and evaluated at runtime with [Kotlin FHIRPath](/fhir-foundations/kotlin-fhirpath/).
...
The repository ships a catalog application with several dozen small Questionnaires, one per component, behavior, and layout, from `component_slider` to `behavior_skip_logic` to `layout_paginated`. It runs on all four platforms and shows each item type and SDC behavior in isolation.

## What it is not

- **Not storage and not sync.** The SDK is purely in-memory. Persisting the `QuestionnaireResponse` (or extracted resources) is your application's job, typically with [Kotlin FHIR Engine](/fhir-foundations/kotlin-fhir-engine/), on which this SDK does not depend.
...
- **Unsupported item types fail loudly.** An unrecognized question type raises rather than rendering a degraded fallback.

## Next step

[Render your first Questionnaire](/fhir-foundations/kotlin-fhir-data-capture/get-started/).
