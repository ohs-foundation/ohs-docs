...
FHIRPath is the expression language FHIR itself is written in. Search parameters, invariants, and Structured Data Capture behavior are all FHIRPath, so an engine for it is the piece that turns static resource data into evaluable logic. In this stack, [Kotlin FHIR Engine](/fhir-foundations/kotlin-fhir-engine/) uses it to index resources for search and [Kotlin FHIR Data Capture](/fhir-foundations/kotlin-fhir-data-capture/) uses it for questionnaire behavior, but it is equally usable on its own.

## What you get

- **Strict conformance to the FHIRPath specification** (v3.0.0, STU3) plus the FHIR-specific additions defined in FHIR R5, verified against the official FHIR test-case suite. Deviations are documented, not silent. See [conformance](/fhir-foundations/kotlin-fhirpath/conformance/).
...
- **Deliberately conservative about timezones.** Comparing a date-time that has a timezone offset with one that has none returns empty rather than assuming a default zone. This is a safety decision for clinical data, explained in [evaluation semantics](/fhir-foundations/kotlin-fhirpath/evaluation-semantics/).
