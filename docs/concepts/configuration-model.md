...
A completed form produces a `QuestionnaireResponse`, which records the answers but is not yet the clinical record. Extraction is the step that turns those answers into the FHIR resources a programme actually queries and reports on, such as a `Patient`, an `Observation`, or a `Group` for a household.
...
A single ViewDefinition projects one resource type. Because real screens show related data, such as a patient with their allergies or a household with its members, `ViewJoinMap` names a **pivot** resource that drives the row count and joins further projections onto each row. Its joins describe where each resource sits in a FHIR search response rather than describing a database join, so a view and the query feeding it have to agree.
...
You reach for code when you need a shape of view state nothing currently draws, such as a new kind of widget, a different arrangement, or an interaction the reference does not have.
